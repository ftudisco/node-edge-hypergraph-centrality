...
correlations = correlation_table(x,y,centralities)
...
for (i,maps) in enumerate(keys(mappings))
    ...
    for (j,maps2) in enumerate(list)
        ...
        plot(x./maximum(x),xx./maximum(xx), "o")
        xlabel("$maps node centrality",fontsize=15)
        ylabel("$maps2 node centrality",fontsize=15)
        xticks(fontsize=13)
        yticks(fontsize=13)
    end
end
...
for (i,maps) in enumerate(keys(mappings))
    ...
    if i == 1 
        ylabel("edge weight",fontsize=15)
    end
    xlabel("$maps edge centrality",fontsize=15)
    xticks(fontsize=13)
    yticks(fontsize=13)
